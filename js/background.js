// TouchTexture class - Handles mouse/touch trails
class TouchTexture {
    constructor() {
        this.size = 64;
...
class GradientBackground {
    constructor(sceneManager) {
        ...
        this.uniforms = {
            ...
            // BRIGHTER COLORS
            uColor1: { value: new THREE.Vector3(0.98, 0.45, 0.15) },  // Brighter Orange #FA7326
            uColor2: { value: new THREE.Vector3(0.08, 0.12, 0.35) },  // Lighter Navy #142058
            uColor3: { value: new THREE.Vector3(0.98, 0.45, 0.15) },
            uColor4: { value: new THREE.Vector3(0.08, 0.12, 0.35) },
            uColor5: { value: new THREE.Vector3(0.98, 0.45, 0.15) },
            uColor6: { value: new THREE.Vector3(0.08, 0.12, 0.35) },
            uSpeed: { value: 1.2 },
            uIntensity: { value: 2.2 },  // INCREASED intensity
            uTouchTexture: { value: null },
            uGrainIntensity: { value: 0.06 },
            uZoom: { value: 1.0 },
            uDarkNavy: { value: new THREE.Vector3(0.06, 0.08, 0.22) }, // Slightly lighter base
            uGradientSize: { value: 0.8 },  // Larger gradients = more visible
            uGradientCount: { value: 6.0 },
            uColor1Weight: { value: 1.2 },  // MORE orange weight
        ...
        const material = new THREE.ShaderMaterial({
          ...
          `,
            ...
            vec3 getGradientColor(vec2 uv, float time) {
              ...
              float mixFactor = max(brightness * 1.5, 0.2);  // Higher base brightness
            ...
            void main() {
              ...
              // STRONGER touch distortion
              vec4 touchTex = texture2D(uTouchTexture, uv);
              float vx = -(touchTex.r * 2.0 - 1.0);
              float vy = -(touchTex.g * 2.0 - 1.0);
              float intensity = touchTex.b;
              uv.x += vx * 1.2 * intensity;  // Increased from 0.8
              uv.y += vy * 1.2 * intensity;
              
              // Stronger ripple effect
              vec2 center = vec2(0.5);
              float dist = length(uv - center);
              float ripple = sin(dist * 25.0 - uTime * 4.0) * 0.06 * intensity;
              float wave = sin(dist * 18.0 - uTime * 3.0) * 0.04 * intensity;
              uv += vec2(ripple + wave);
              ...
              color.r += sin(timeShift) * 0.03;
              color.g += cos(timeShift * 1.4) * 0.02;
              color.b += sin(timeShift * 1.2) * 0.03;

              float brightness = length(color);
              float mixFactor = max(brightness * 1.3, 0.2);
              color = mix(uDarkNavy, color, mixFactor);
...
class App {
    ...
    triggerFeedback(isCorrect) {
        const targetColor = isCorrect
            ? new THREE.Vector3(0.31, 0.78, 0.47)
            : new THREE.Vector3(0.8, 0.0, 0.12);

        const uniforms = this.gradientBackground.uniforms;
        const originalColor1 = uniforms.uColor1.value.clone();
        ...
        const step = () => {
            ...
            uniforms.uColor1.value.lerpVectors(originalColor1, targetColor, progress);
            uniforms.uColor3.value.lerpVectors(originalColor1, targetColor, progress);
            if (progress < 1) {
...
window.initBackground = () => {
    window.bgApp = new App();
    window.bgConfig = {
        triggerFeedback: (isCorrect) => window.bgApp.triggerFeedback(isCorrect)
    };
};
